...
! READ THE NUMBER OF REDUCIBLE K-POINTS
   ...
   if (symm%lnsymmr) then
      ...
      if (.not. allocated(symm%symop_id)) allocate(symm%symop_id(4,itmp2))
   else
      allocate(cktmp2(3,itmp))
      if (.not. allocated(symm%symop_id)) allocate(symm%symop_id(3,itmp))
   endif
   ...
   if (symm%lnsymmr) then
      ...
      do isym=1,symm%nsym
         do k=1, size(cktmp,2)
            ...
            do j=1,3
               cktmp3(j,i)=cktmp(j,k)*exp(-2.0d0*pi*ci*dot_product(G0(:,6),symm%Tras(:,isym)))
