...
def h5output_hk(outfile, escalc, velcalc=None, peierls=False):
  '''
  Output function which reads in the elements from the specific Hk class into a hdf5 file.
  This allows for specific characteristics to be predefined 
  ...
  with h5py.File(outfile, 'w') as h5out:
    ...
    h5out['.unitcell/nsym']       = 1 #only loading in reducible k-meshes #escalc.nsym
    h5out['.unitcell/symop']      = [[1,0,0],[0,1,0],[0,0,1]] #escalc.symop
    ...
    for ispin in range(escalc.spins):
      ...
      # energies
      h5out[prefix+'energies'] = escalc.energies[0]
      ...
      if escalc.opticfull:
        for ikp in range(escalc.nkp):
          ...
          h5out[prefix+'kPoint/{:010}/BerryCurv'.format(ikp+1)] = escalc.berry_curv[ispin][ikp]
